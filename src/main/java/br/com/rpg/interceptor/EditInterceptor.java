...
public class EditInterceptor implements Interceptor {

	private Result result;
	private final Validator validator;
	private final Logger logger = LoggerFactory.getLogger(EditInterceptor.class);

	public EditInterceptor(Result result, Validator validator) {
		this.result = result;
	...
	public void intercept(InterceptorStack stack, ResourceMethod method, Object resourceInstance) {
		...
		try {
			...
			if (e.getCause().getClass().equals(ConstraintViolationException.class)) {
				...
				validator.add(new I18nMessage("error", "error.cannotEdit"));
			}
		} finally {
			if (!validator.hasErrors()) {
				result.include("success", "editSuccess");
			}
			if (!result.used()) {
	...
	public boolean accepts(ResourceMethod method) {
		if (method.containsAnnotation(Post.class)) {
			return method.getMethod().getName().equals("edit");
		}
		return false;
	}
}